...
# with open(r"C:\Users\Filippo Calavaro\Documents\Filippo Calavaro\Data\20250722\scanZ_metadata.json",
#           "r") as NF_spectra_scanZ:
#     NF_spectra_scanZ_datasets = json.load(NF_spectra_scanZ)
#
# # Loop over all datasets
# for selected_dataset in NF_spectra_scanZ_datasets:
#     # Read all spectra for the current dataset
#     all_spectra_dict = read_all_spectraNF(selected_dataset["folder"])
#
#     # Read reference spectrum and process background subtraction
#     ref_df = read_spectrum_txt_to_dataframe(Path(selected_dataset["folder"]) / "ref.txt")
#     background_region = ref_df[(ref_df["Wavelength_nm"] >= 843) & (ref_df["Wavelength_nm"] <= 844)]
#     background_mean = background_region["Intensity_counts"].mean()
#     ref_df["Intensity_counts"] -= background_mean
#     ref_df["Intensity_counts"] = ref_df["Intensity_counts"].clip(lower=0)
#
#     # Print the spectra dictionary for the current dataset
#     print(f"Spectra dictionary for dataset: {selected_dataset['folder']}")
#     print(all_spectra_dict)
#
#     # Plot spectra heights for the current dataset
#     plot_spectra_heights(
#         all_spectra_dict,
#         integration_time=selected_dataset["integration_time"],
...
#     plt.show()
#
#     # Plot reference spectrum for the current dataset
#     plot_single_spectrum(ref_df, title=f"Reference spectrum for dataset:\n{selected_dataset['folder']}")
#
#     # Uncomment if you want to include whitelightref.txt processing
#     # whitelight_df = read_spectrum_txt_to_dataframe(Path(selected_dataset["folder"]).parent / "whitelightref.txt")
#     # plot_single_spectrum(whitelight_df, title=f"Whitelight spectrum for dataset: {selected_dataset['folder']}")
#
#     # Generate integral map for the current dataset
#     heights, wl_bins, intensity_map = integral_map_different_heights(
#         spectra_dict=all_spectra_dict,
#         integration_time=selected_dataset["integration_time"],
#         wl_start=759,
...
#     plt.title(f"2D map: integrated counts per 1 nm bin for dataset:\n{selected_dataset['folder']}", fontsize=18, pad=15)
#
#     # Ticks font size
#     plt.xticks(fontsize=14)
#     plt.yticks(fontsize=14)
