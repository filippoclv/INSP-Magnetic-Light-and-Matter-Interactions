...
def fMapPowerCurves(PiezoStage, RotorStage, PowerMeter, 
    ...
    """
    Performs a 2D map relative to the manual starting position (Auto-Anchor).
    - Auto-detects current position as Origin (0,0).
    - Scans offsets relative to this Origin.
    - SAFETY: Returns to the Origin at the end (or if crash occurs).
    - SAFETY: Clamps Power to calibration limits to prevent 'NaN' crashes.
    """
    ...
    # 4. Calculate Safety Limits from Calibration
    ...
    # Clamp slightly inside physics limits to prevent NaN errors
    ...
    try: 
        ...
        for iY, Y_Offset in enumerate(Y_Rel_List):
            # Calculate Absolute Target based on ANCHOR
        ...
        try:
            # 1. Reset Laser
            print("1. Resetting Laser to Minimum Power...")
            Offset = PowerRangeFitParameters[2]
            SafeMin = Offset * 1.001
            fMoveToPower(RotorStage, PowerMeter, SafeMin, *PowerRangeFitParameters)
            print("   -> Laser Safe.")
            
            # 2. Reset Piezo to ANCHOR
            print(f"2. Returning Piezo to Anchor ({OriginX:.4f}, {OriginY:.4f})...")
            PiezoStage.MOVEX(OriginX, "u")
            PiezoStage.MOVEY(OriginY, "u")
            print("   -> Piezo Returned to Start.")
...
def GetPointSpectro(Power, 
    ...
    if Lum < NoiseMean / 5:
        ...
        GetPointSpectro(Power, SpectrumName, MyDataFolder, DelayIntegrationTime, NoiseMean)
        return None

    with open(MyDataFolder + '\\' + 'SetInfoPowerCurve.txt', 'a') as FileInfo:
        FileInfo.write(f'{Power}\t{CurrentPower}\t{CurrentTime}\t{Lum}\n')
    if SpectrumName[0] != "D":
        plt.plot(CurrentPower, Lum + NoiseMean, 'r+')
    return CurrentPower, Lum

def GetPointAPD(Power,
                MyDataFolder,
                IntegrationTime = 0.5, 
                NoiseMean = 0):
    #Move to power
    ...
    #Save point
    with open(MyDataFolder + '\\' + 'SetInfoPowerCurve_APD.txt', 'a') as FileInfo:
        FileInfo.write(f'{Power}\t{CurrentPower}\t{CurrentTime}\t{CountRateToWrite}\n')
    plt.plot(CurrentPower, CountRateToWrite, 'r+')
    return CurrentPower, CountRateToWrite
...
def FindAvalanche(
    ...
    rough_step_gap = (Pmax - Pmin) / 50 #First measurement and gap between measurements
    fine_step_gap = rough_step_gap /10 #Resolution of the avalanche
    PowerLimit = Pmax / 3 #Safety
    LumQueueSize =  int(2 * rough_step_gap // fine_step_gap)
    SlopeQueueSize = 3
    integration_time = 0 #Placeholder
    nbdark = 6
    ...
    for j in range(LumQueueSize - 2):
        ...
        if Spectro:
            ...
            LastNPump[j], LastNLum[j] = GetPointSpectro(Power + (2 - LumQueueSize + j) * fine_step_gap, SpectrumName, MyDataFolder, integration_time, noise[0])
        else:
            LastNPump[j], LastNLum[j] = GetPointAPD(Power + (2 - LumQueueSize + j) * fine_step_gap, MyDataFolder, integration_time, noise[0])
...
def FindAvalanchePowscale(
    ...
    fine_step_ratio = 10
    fine_step_gap = np.power(10, np.log10(rough_step_gap) / fine_step_ratio) #Resolution of the avalanche
    PowerLimit = Pmax / 3 #Safety
    LumQueueSize = int(2 * fine_step_ratio)
    SlopeQueueSize = 3
    nbdark = 10
...
if 'PowerRangeFitParameters' not in locals():
    ...
    # 2. SETUP SPATIAL GRID (Microns RELATIVE to current position)
    # 
    # Examples:
    # np.linspace(0, 2.0, 3)   -> Scans [0.0, 1.0, 2.0] relative to start
    # np.linspace(-1.0, 1.0, 3) -> Scans [-1.0, 0.0, 1.0] centered on start
    X_Rel = np.linspace(0, 2.0, 3)   
    Y_Rel = np.linspace(0, 2.0, 3)   

    # 3. SETUP POWER (Ratios)
    # We calculate Watts from calibration to avoid crashing
