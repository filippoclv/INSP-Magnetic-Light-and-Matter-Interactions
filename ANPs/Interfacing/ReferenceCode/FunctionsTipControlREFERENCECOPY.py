...
def BoucleGetSpectra(NumberSpectra, DelayIntegrationTime, FolderName):
...
def ChangeSetPointV(MoveSenZ_mV):
    if MoveSenZ_mV > 0:
        print('Le SenZ doit être négatif !')
        return
...
def FromPhaseToSenZ():
    ...
    # On demande de rentrer la valeur actuelle de SenZ et on modifie le SetPoint:
    CurrentSenZ_mV = pyGUI.prompt(text='Quelle est la valeur du SenZ ?', title='Attention !' , default='')
