...
def FindAvalanchePowscale(
    ...
    print("Autodetection is running ...")
    rough_step_gap = 1.1 #First measurement and gap between measurements
    fine_step_ratio = 10
    ...
    for j in range(LumQueueSize - 2):
        index += 1 
        SpectrumName = f"Avalanche{index}"
        LastNPump[j], LastNLum[j] = GetPointSpectro(Power * fine_step_gap ** (2 - LumQueueSize + j), SpectrumName, MyDataFolder)
...
LumQueueSize = FindAvalanchePowscale(
            SaveDataFolder,
            Pmin,
            Pmax,
            0.001,
